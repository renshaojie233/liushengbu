...
async function transcribeAudioFile(config, inputFile) {
  ...
  const estimatedAudioMs = Math.max(
    0,
    Math.ceil(((audio.length - (protocol === "v3" ? 44 : 0)) / 32000) * 1000)
  );
  const sessionTimeoutMs = Number(
    config.fileSessionTimeoutMs || Math.max(30 * 60 * 1000, estimatedAudioMs + 10 * 60 * 1000)
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const armSessionTimer = () => {
      ...
      sessionTimer = setTimeout(() => {
        ...
        fail(new Error("豆包语音文件转写超时，请稍后重试或改用录音文件识别接口。"));
      }, sessionTimeoutMs);
    };
